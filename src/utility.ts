...
import { PATH_ITEM_DELIMITER, DYNAMIC_PATH_PREFIX, OPEN_SWITCH_EXPR_SYMBOL, CLOSE_SWITCH_EXPR_SYMBOL, SINGLE_QUOTE_LITERAL, DOUBLE_QUOTE_LITERAL, STOP, SPREAD_PATH_PREFIX, SKIP_matchRuntimeAndDefinedSwitches } from "./constants";
 ...
 * @returns {void}
 */
export function matchRuntimeAndDefinedSwitches(pathItem: PathItem, argsShortSwitches: Record<string, string[]>, argsLongSwitches: Record<string, string[]>, config: Config, context: CallbackContext) {

  /*********************************************************************
   * Check if helpType is switch and help switch exists                *
   *********************************************************************/

  if (context[SKIP_matchRuntimeAndDefinedSwitches]) {
    return;
  }
 ...
 */
export function treeToString (pathItem: PathItem, shortForm: boolean = false) {
  let output = '';
